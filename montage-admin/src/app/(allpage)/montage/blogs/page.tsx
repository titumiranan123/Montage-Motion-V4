...
import TiptapEditor from "./CustomEditor";
...
const Blogs = () => {
  ...
  const [parent, tapes, setTapes] = useDragAndDrop<HTMLDivElement, IBlog>(
    blogs
  );
  ...
  return (
    <div className="min-h-screen   text-gray-100 p-4 md:p-8">
      <div className="max-w-7xl mx-auto">
        {/* Header */}
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4 border-b border-[#1FB5DD] rounded-lg pb-3 ">
          <div>
          ...
          <div className="flex gap-3">
            ...
            <button
              ...
              className="bg-[#1FB5DD]  text-white font-medium py-2 px-4 rounded-lg flex items-center gap-2"
            >
              <FaPlus /> Add New Blog
            </button>
          </div>
          ...
          {isLoading ? (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {[...Array(6)].map((_, idx) => (
                <div
                  key={idx}
                  className="flex flex-col md:flex-row gap-4 p-4 border border-gray-200 rounded-lg shadow-sm bg-white w-[600px] animate-pulse"
                >
                  {/* Image Skeleton */}
