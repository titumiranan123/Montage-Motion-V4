...
export interface ITestimonial {
  ...
  video_message?: string; // Made optional
  message?: string;
  position?: number;
...
const TestimonialForm: React.FC<ITestimonialFormProps> = ({
  ...
  const handleThumbnailUpload = useCallback(
    async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      try {
        ...
        await Swal.fire("Upload Failed", "Failed to upload thumbnail", "error");
      } finally {
  ...
  const onSubmitHandler = async (data: ITestimonial) => {
    try {
      // Remove video_message and thumbnail if category is "message"
      const submitData = {
        ...data,
        ...(data.category === "message"
          ? {
              video_message: undefined,
              thumbnail: undefined,
            }
          : {}),
      };
      let res;
      if (submitData.id) {
        res = await api_url.put(
          `/api/testimonials/${submitData.id}`,
          submitData,
        );
      } else {
        res = await api_url.post("/api/testimonials", submitData);
      }
        ...
        >
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              {/* Video and Thumbnail Upload - Only shown when category is video_message */}
                ...
                <>
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      >
                        <div className="flex flex-col items-center justify-center p-6 border-2 border-dashed border-gray-600 rounded-xl hover:border-amber-500 transition group">
                          {thumbnailPreview ? (
                            <div className="relative w-full">
                              <img
                            ...
                            <>
                              ...
                              <p className="text-sm text-gray-400 text-center">
                                <span className="font-medium text-amber-500 hover:text-amber-400 transition">
                                  Click to upload
                                </span>{" "}
                                or drag and drop
                              </p>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        className="w-full bg-gray-700 text-white rounded-lg px-4 py-3 border border-gray-600 focus:border-amber-500 focus:ring-2 focus:ring-amber-500 focus:outline-none transition"
                      />
                    </div>
                  </div>
                </>
          ...
          {/* Text Message Section - Only shown when category is message */}
          {category === "message" && (
            <div className="space-y-4">
              <h2 className="text-xl font-semibold text-white border-b border-gray-700 pb-2">
                Testimonial Content
          ...
          <div className="flex justify-end gap-4 pt-6 border-t border-gray-700">
            ...
            <button
              ...
              className="px-6 py-3 bg-[#1FB5DD] text-white rounded-lg hover:bg-[#1a9fc2] transition focus:outline-none focus:ring-2 focus:ring-[#1FB5DD] focus:ring-offset-2 focus:ring-offset-gray-800 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {isSubmitting ? (
